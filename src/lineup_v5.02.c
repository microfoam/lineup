...
printf("\n Seq=%s", Seq);
	...
	if (!opt_s.bit) {			/* ONLY IF opt_s OPTION TO SILENCE OUTPUT IS NOT ON */
		...
		fprintf(fp_out, "v%s\t%.20s\tx%d\t%.3f\t%.3f\t%.3f\tP:%4d R:%4d %s (%c%d %s)\t%s\n", 
				version, time0+4, opt_x.val, ratio1, ratio2, ratio3, Current.pass_Q, Recover.pass_Q, 
		...
		if (Current.pass_Q != 1000 || (opt_R.bit && Recover.pass_Q != 1000)) {
			...
			fprintf(fp_tricksy, "v%s\t%.20s\tx%d\t%.3f\t%.3f\t%.3f\tP:%4d R:%4d %s (%c%d %s)\t%s\n", 
					version, time0+4, opt_x.val, ratio1, ratio2, ratio3, Current.pass_Q, Recover.pass_Q, 
					file_name, Strand->sym, Clean.pass_W, letr_unit, dev_notes);
			for(n = 0; n<lenseq; n++) {
